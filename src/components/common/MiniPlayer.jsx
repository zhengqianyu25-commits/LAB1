      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 border-t" style={{ background: 'rgba(255,255,255,0.97)', borderColor: 'var(--color-border)', backdropFilter: 'blur(20px)' }}>
        ...
        <div className="px-4 pt-2 pb-0">
          <input
            ...
            style={{
              background: `linear-gradient(to right, var(--color-accent) ${duration ? (currentTime/duration)*100 : 0}%, var(--color-border) ${duration ? (currentTime/duration)*100 : 0}%)`,
        ...
        <div className="container-main flex items-center gap-3 py-1.5">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button onClick={prev} className="w-7 h-7 flex items-center justify-center rounded-full transition-colors opacity-50 hover:opacity-100" style={{ color: 'var(--color-text)' }} aria-label="Previous">
            ...
            <button onClick={next} className="w-7 h-7 flex items-center justify-center rounded-full transition-colors opacity-50 hover:opacity-100" style={{ color: 'var(--color-text)' }} aria-label="Next">
            ...
            <button onClick={addToQueue} className="w-7 h-7 flex items-center justify-center rounded-full transition-colors" style={{ color: inQueue ? 'var(--color-accent)' : 'var(--color-text-muted)', opacity: inQueue ? 1 : 0.4 }} aria-label="Add to queue">
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
            </button>
          </div>

          <button onClick={() => setShowQueue(!showQueue)} className="w-7 h-7 shrink-0 flex items-center justify-center rounded-full opacity-40 hover:opacity-80 transition-opacity" style={{ color: 'var(--color-text-muted)' }} aria-label="Queue">
          ...
          {queue.length === 0 ? (
            ...
            queue.map((q, i) => (
              <div key={i} className={`queue-row flex items-center gap-3 px-4 py-2.5 transition-colors cursor-pointer hover:bg-[var(--color-bg-secondary)] ${episode.slug === q.slug ? 'border-l-[3px]' : ''}`}
                ...
                onClick={() => {
                  const found = audioArticles.findIndex(a => a.slug === q.slug);
