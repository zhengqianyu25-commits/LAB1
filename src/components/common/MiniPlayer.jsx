    ...
    <>
      <div className="fixed bottom-0 left-0 right-0 z-50 border-t" style={{ background: 'rgba(255,255,255,0.98)', borderColor: 'var(--color-border)', backdropFilter: 'blur(20px)' }}>
        <div className="flex items-center px-4 py-2 gap-3">
          {/* Left: cover + title */}
          <div className="flex items-center gap-3 min-w-0 flex-[2] cursor-pointer" onClick={() => navigate(`/listen/${idx}`)}>
          ...
          {/* Right: controls */}
          <div className="flex items-center gap-1.5 flex-[2] justify-end">
            ...
            <button onClick={prev} className="w-6 h-6 flex items-center justify-center rounded-full opacity-50 hover:opacity-100" style={{ color: 'var(--color-text)' }} aria-label="Previous">
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="m19 20 1 0V4l-1 0v16zM4.5 12l10-6v12l-10-6z"/></svg>
            </button>
            ...
            <button onClick={next} className="w-6 h-6 flex items-center justify-center rounded-full opacity-50 hover:opacity-100" style={{ color: 'var(--color-text)' }} aria-label="Next">
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M5 4h1v16H5V4zm14.5 8-10 6V6l10 6z"/></svg>
            </button>
            <button onClick={() => setShowQueue(!showQueue)} className="w-6 h-6 flex items-center justify-center rounded-full opacity-40 hover:opacity-80" style={{ color: 'var(--color-text-muted)' }} aria-label="Queue">
